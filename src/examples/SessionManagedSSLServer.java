...
public class SessionManagedSSLServer implements HandshakeCompletedListener, Runnable {
    ...
    private SSLServerSocket serverSocket; // Replace with your own local values … 
    ...
    public SessionManagedSSLServer(int port) throws IOException, GeneralSecurityException {
        if (sslContext == null) {
            ...
            sslContext.init(kmf.getKeyManagers(), null, null); // Configure the server session context (1.4-specific) 
    ...
    public void handshakeCompleted(HandshakeCompletedEvent event) {
        ...
        try {
            ...
            // check principal.getName() &c against expectations, // not shown …
        } catch (SSLPeerUnverifiedException exc) { // do whatever is required, e.g. close the socket } 
    ...
    class ConnectionThread extends Thread {
        ...
        @Override
        public void run() {
            try { // Choose whichever of these suits your situation ... 
